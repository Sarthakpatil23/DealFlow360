...
2. **Init Prisma, add schema.prisma** (from `schema.md`). Test: `npx prisma format` runs with no errors.
3. **Setup local Postgres via Docker.** Test: `docker ps` shows the container running.
4. **Run first migration** (`prisma migrate dev --name init`). Test: tables exist, check with `npx prisma studio`.
5. **Write seed.ts** (Acme Corp, sample products, tiers, one warehouse). Test: `npx prisma db seed` populates data, visible in Prisma Studio.
